...
数组模式匹配类数组对象（拥有`length`属性的对象）。剩余元素绑定（可以是匿名的）用于匹配任意长度的数组。例子：
...
第三，更加一般地，它围绕数值间的互相匹配创建了一个协议。这在未来的提案中也许会很有用，比如`interface`提案，用于添加类似于nominal interface/tagged union discrimination的东西。
...
Fall-through可以通过`continue`关键字达到。如果没有匹配的模式，这将会是一个运行时的错误。
...
对于匹配体的语法存在许多选择，大体上说有：类case语法，类箭头函数语法，以及只包含表达式的语法。

#### 类Case语法的分支

类Case语法的分支包含声明。这种分支的后项按一个声明接一个声明的顺序执行直到遇到流程控制关键字（或者达到case结构的末尾）。类Case的分支非常有用因为它们允许声明做为子元素，`throw`声明就经常会用到。

类Case的分支在语句构成上比较难处理，因为你需要一个关键字来表示一个分支的开始。一个显而易见的选择是使用`case`。找到其他符合语境的关键字会比较困难，但大概也不是完全不可能。

另外，由于`match`表达式的值是第一个匹配分支执行后的值，模式匹配的用户将不得不理解语义上不是JS开发者通常所认为的`completion value`。

最后，类case的分支在更小的场景中使用模式匹配显得繁琐。

#### 类箭头函数分支

箭头函数支持一个表达式或者一个可选的语句块。把这应用到我们的模式匹配的语法带来了两个很好的特性：简洁而不失可拓展型，和用`,`分割的分支。这个方案应用到了上述所有例子中。
